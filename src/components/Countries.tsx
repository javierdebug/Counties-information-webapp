...
export default function Countries({countryFilter, continentFilter, currencyFilter}: CountriesInterface) {
  ...
  if (currencyFilter) {
    countriesDatabase = data.countries.filter((country: Country) => {
      if (country.currency) {
        return country.currency.toLowerCase().trim().includes(currencyFilter.toLowerCase().trim());
      }
    })
  };
  ...
  if (currencyFilter && continentFilter) {
    countriesDatabase = data.countries.filter((country: Country) => {
      if (country.currency) {
        if (country.currency.toLowerCase().trim().includes(currencyFilter.toLowerCase().trim()) && country.continent!.name === continentFilter) {
          return true
        }
      }
    });
